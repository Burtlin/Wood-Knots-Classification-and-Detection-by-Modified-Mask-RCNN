...
def rpn_class_loss_graph(rpn_match, rpn_class_logits):
    ...
    gamma = 2
    labels = tf.one_hot(targets, tf.shape(logits)[-1])
    softmax = tf.nn.softmax(logits)
    log = tf.log(softmax)
    loss = -tf.reduce_sum(labels * tf.pow(1. - softmax, gamma) * log, 1)
    
    loss = K.switch(tf.size(loss) > 0, K.mean(loss), tf.constant(0.0))
    loss = 50 * loss  #@change@#
    #print("loss = 1 * loss")  #@change@#
    return loss
